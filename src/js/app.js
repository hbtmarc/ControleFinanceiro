    ...
    try { window.appApi = window.appApi || {}; window.appApi.saveVtEntry = saveVtEntry; window.appApi.updateVtEntry = updateVtEntry; window.appApi.render = function(){ try { if (typeof render === 'function') return render(); } catch(e){ console.error('appApi.render wrapper error', e); } }; } catch(e) { /* ignore env failures */ }
                ...
                state.appData.settings.paymentMethods.forEach(pm => {
                    if (pm.type === 'credit_card') {
                        if (pm.name === 'Nubank' && !pm.closeDay) {
                            pm.closeDay = 3;
                            pm.dueDay = 10;
                        } else if (pm.name === 'Porto' && !pm.closeDay) {
                            pm.closeDay = 31;
                            pm.dueDay = 11;
                        }
                    }
        ...
        function mapUI() {
            const ids = [
                ...
                'expense-fields', 'dashboard-view', 'credit-cards-view', 
                ...
                'desktop-dashboard-btn', 'desktop-creditcards-btn',
        ...
        function bindEvents() {
            ...
            if (ui.transactionModal) attachBackdropDoubleClickToClose(ui.transactionModal, 'transaction-modal');
                        ...
                        months.forEach(m => {
                            ...
                            html += `<div class="flex items-center justify-between p-2 rounded bg-gray-50 border">
                                ...
                                <div class="text-xs grid grid-cols-3 gap-2 text-right" style="min-width:320px">
                                    <div class="text-xs">In: ${formatCurrency(bIn)} â†’ ${formatCurrency(cIn)}</div>
                                    <div class="text-xs">Ex: ${formatCurrency(bEx)} â†’ ${formatCurrency(cEx)}</div>
                                    <div class="text-xs">Sa: ${formatCurrency(bSa)} â†’ ${formatCurrency(cSa)}</div>
                                </div>
                            </div>`;
                        });
                    });
            ...
            if (ui.annualReportBtn) ui.annualReportBtn.addEventListener('click', () => openAnnualReport());
            if (ui.closeMonthlyReportBtn) ui.closeMonthlyReportBtn.addEventListener('click', () => closeModal('monthly-report-modal'));
            if (ui.closeAnnualReportBtn) ui.closeAnnualReportBtn.addEventListener('click', () => closeModal('annual-report-modal'));
            if (ui.addGoalBtn) ui.addGoalBtn.addEventListener('click', () => openGoalModal());
            if (ui.closeGoalModalBtn) ui.closeGoalModalBtn.addEventListener('click', () => closeModal('goal-modal'));
            if (ui.goalForm) ui.goalForm.addEventListener('submit', (e) => handleGoalFormSubmit(e));
            if (ui.searchDescription) ui.searchDescription.addEventListener('input', (e) => {
        ...
        function renderCreditCardView() {
            ...
            creditCards.forEach(card => {
                    ...
                    <div class="card p-6 rounded-2xl shadow-sm flex flex-col" data-card-root="${card.name}">
                        <div class="flex-grow">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex items-center text-sm text-gray-500" style="gap:6px;">
                                        <button class="card-sort-arrow" data-card-name="${card.name}" data-sort="value-desc" title="Ordenar por valor desc"><span class="material-symbols-outlined" style="font-size:16px">arrow_upward</span></button>
                                ...
                                <div class="flex items-center gap-2">
                                    <span class="text-xs font-medium bg-orange-100 text-orange-800 px-2 py-1 rounded-full">Fecha ${closeDate.toLocaleDateString('pt-BR', {day: '2-digit', month: '2-digit'})}</span>
                            ...
                            <div class="flex items-center gap-4 text-sm text-gray-500 mt-1 mb-2">
                                <div class="flex items-center gap-1"><span class="material-symbols-outlined" style="font-size:16px">event_busy</span><span>Fech. dia ${closeDay}</span></div>
                                <div class="flex items-center gap-1"><span class="material-symbols-outlined" style="font-size:16px">calendar_today</span><span>Venc. dia ${cardDueDay}</span></div>
                ...
                try {
                    ...
                    nonCardPayments.forEach(pm => {
                        ...
                        if (!pmName) return;
                        if (state.cardGrouped[pmName] === undefined) state.cardGrouped[pmName] = false;
                    });
                    nonCardPayments.forEach(pm => {
                        const pmName = (typeof getPaymentLabel === 'function') ? getPaymentLabel(pm) : ((typeof pm === 'string') ? pm : (pm.name || pm.label || pm.value || ''));
                        if (!pmName) return;
                        ...
                        if (!state.cardGrouped[pmName]) {
                            if (sortMode === 'alf') {
                                pmExpenses = pmExpenses.slice().sort((a,b) => { const A=(a.description||'').toLowerCase(); const B=(b.description||'').toLowerCase(); return A < B ? -1 : A > B ? 1 : 0; });
                            } else if (sortMode === 'value-desc') {
                                pmExpenses = pmExpenses.slice().sort((a,b) => (Number(b.amount)||0) - (Number(a.amount)||0));
                            } else if (sortMode === 'value-asc') {
                        ...
                        container.innerHTML += `
                            <div class="card p-6 rounded-2xl shadow-sm flex flex-col" data-card-root="${pmName}" data-payment-method="${pmName}">
                                <div class="flex-grow">
                                    <div class="flex justify-between items-start mb-2">
                                        <div class="flex items-center gap-3">
                                            <h3 class="text-xl font-semibold card-title-sortable cursor-pointer" data-card-name="${pmName}">${pmName}</h3>
                                            <div class="flex items-center text-sm text-gray-500" style="gap:6px;">
                                                <button class="card-sort-arrow" data-card-name="${pmName}" data-sort="value-desc" title="Ordenar por valor desc"><span class="material-symbols-outlined" style="font-size:16px">arrow_upward</span></button>
                                    ...
                                    <div class="space-y-1 max-h-48 overflow-y-auto pr-2 card-items-list" data-card-name="${pmName}"></div>
                                </div>
                                <div class="mt-4">
                                    <div class="flex w-full items-center" style="gap:8px;">
            ...
            container.querySelectorAll('.group-header').forEach(h => h.addEventListener('click', (ev) => {
                ...
                if (root) {
                    ...
                    if (caret) caret.textContent = (!current) ? 'â–¸' : 'â–¾';
                }
            }));
                        ...
                        unified.forEach(item => {
                            if (item.type === 'group') {
                                ...
                                if (btn) {
                                    ...
                                    btn.addEventListener('click', (ev) => { ev.stopPropagation(); const lines = items.map(it => `${formatCurrency(it.amount)} â€” ${escapeHtml(it.description || '')}`); showModalList(`Detalhes: ${key}`, lines.join('\n')); });
                                }
                    ...
                    listEl.querySelectorAll('.edit-cat-btn').forEach(btn => btn.addEventListener('click', (ev) => {
                        ...
                        if (groupKeyEnc) {
                            ...
                            if (items && items.length > 0) {
                                const lines = items.map(it => `${formatCurrency(it.amount)} â€” ${escapeHtml(it.description || '')}`);
            ...
            unified.forEach(item => {
                if (item.type === 'group') {
                    ...
                    setTimeout(() => {
                        ...
                        infoBtn.addEventListener('click', (ev) => { ev.stopPropagation(); const lines = items.map(it => `${formatCurrency(it.amount)} â€” ${escapeHtml(it.description || '')} (${escapeHtml(it.payment || it.category || '')})`); showModalList(`Detalhes: ${key}`, lines.join('\n')); });
